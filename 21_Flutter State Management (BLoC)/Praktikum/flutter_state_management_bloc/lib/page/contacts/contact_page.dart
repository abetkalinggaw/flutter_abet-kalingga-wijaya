...
class ContactsPage extends StatefulWidget {
  const ContactsPage({Key? key, required List<Contact> contacts})
      : super(key: key);
...
class _ContactsPageState extends State<ContactsPage>
  ...
  late ContactsProvider _provider;
  ...
  Future<void> _openCreateContactPage(
    ...
    if (result != null) {
      setState(() {
        final index = _provider.contacts
            .indexWhere((element) => element.name == result.name);
        if (index != -1) {
          _provider.updateContact(result, index);
          _listKey.currentState!.insertItem(index);
        } else {
          _provider.addContact(result);
          _listKey.currentState!.insertItem(_provider.contacts.length - 1);
        }
      });
    }
  }
  ...
  void initState() {
    ...
    _provider = Provider.of<ContactsProvider>(context, listen: false);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: AnimatedList(
        key: _listKey,
        initialItemCount: _provider.contacts.length,
...
class _CreateContactPageState extends State<CreateContactPage>
  ...
  void _createContact() {
    ...
    if (name.isNotEmpty && phone.isNotEmpty) {
      ...
      _provider.addContact(newContact);
  ...
  @override
  Widget build(BuildContext context) {
    _provider = Provider.of<ContactsProvider>(context);
...
class _UpdateContactPageState extends State<UpdateContactPage>
  ...
  void _updateContact() {
    ...
    if (name.isNotEmpty && phone.isNotEmpty) {
      final newContact = Contact(name, phone);
      final index = _provider.contacts.indexOf(widget.contact);
      _provider.updateContact(newContact, index);
      Navigator.of(context).pop(newContact);
    }
  }
